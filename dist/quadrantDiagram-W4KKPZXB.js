import { s as Ae, g as ke, t as ae, q as Fe, a as Pe, b as ve, _ as o, c as zt, l as bt, d as Lt, e as Ce, A as Le, I as z, i as Ee, a1 as De } from "./index.js";
import { l as ie } from "./linear.js";
var Et = (function() {
  var t = /* @__PURE__ */ o(function(Y, r, l, x) {
    for (l = l || {}, x = Y.length; x--; l[Y[x]] = r) ;
    return l;
    ...
    parse: /* @__PURE__ */ o(function(r) {
      var l = this, x = [0], c = [], S = [null], e = [], ut = this.table, s = "", yt = 0, Jt = 0, me = 2, $t = 1, be = e.slice.call(arguments, 1), D = Object.create(this.lexer), j = { yy: {} };
      for (var Ft in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, Ft) && (j.yy[Ft] = this.yy[Ft]);
      D.setInput(r, j.yy), j.yy.lexer = D, j.yy.parser = this, typeof D.yylloc > "u" && (D.yylloc = {});
      var Pt = D.yylloc;
      e.push(Pt);
      ...
      function _e(R) {
        x.length = x.length - 2 * R, S.length = S.length - R, e.length = e.length - R;
      }
      o(_e, "popStack");
      function te() {
        var R;
        return R = c.pop() || D.lex() || $t, typeof R != "number" && (R instanceof Array && (c = R, R = c.pop()), R = l.symbols_[R] || R), R;
      }
      o(te, "lex");
      for (var B, G, W, vt, rt = {}, Tt, M, ee, qt; ; ) {
        if (G = x[x.length - 1], this.defaultActions[G] ? W = this.defaultActions[G] : ((B === null || typeof B > "u") && (B = te()), W = ut[G] && ut[G][B]), typeof W > "u" || !W.length || !W[0]) {
          var Ct = "";
          qt = [];
          for (Tt in ut[G])
            this.terminals_[Tt] && Tt > me && qt.push("'" + this.terminals_[Tt] + "'");
          D.showPosition ? Ct = "Parse error on line " + (yt + 1) + `:
` + D.showPosition() + `
Expecting ` + qt.join(", ") + ", got '" + (this.terminals_[B] || B) + "'" : Ct = "Parse error on line " + (yt + 1) + ": Unexpected " + (B == $t ? "end of input" : "'" + (this.terminals_[B] || B) + "'"), this.parseError(Ct, {
            text: D.match,
        ...
        switch (W[0]) {
          case 1:
            x.push(B), S.push(D.yytext), e.push(D.yylloc), x.push(W[1]), B = null, Jt = D.yyleng, s = D.yytext, yt = D.yylineno, Pt = D.yylloc;
            break;
          case 2:
            if (M = this.productions_[W[1]][1], rt.$ = S[S.length - M], rt._$ = {
              ...
              return vt;
            M && (x = x.slice(0, -1 * M * 2), S = S.slice(0, -1 * M), e = e.slice(0, -1 * M)), x.push(this.productions_[W[1]][0]), S.push(rt.$), e.push(rt._$), ee = ut[x[x.length - 2]][x[x.length - 1]], x.push(ee);
            break;
          case 3:
            return !0;
        }
...
var ze = Et, I = De(), ot, Ve = (ot = class {
  ...
  calculateSpace(n, f, d, h) {
    ...
    }, k = this.config.titleFontSize + this.config.titlePadding * 2, u = {
      top: h ? k : 0
    }, T = this.config.quadrantPadding + a.left, q = this.config.quadrantPadding + y.top + u.top, m = this.config.chartWidth - this.config.quadrantPadding * 2 - a.left - a.right, b = this.config.chartHeight - this.config.quadrantPadding * 2 - y.top - y.bottom - u.top, g = m / 2, A = b / 2;
    return {
      xAxisSpace: y,
      yAxisSpace: a,
      titleSpace: u,
  ...
  getAxisLabels(n, f, d, h) {
    ...
    return this.data.xAxisLeftText && f && g.push({
      ...
      x: this.config.yAxisPosition === "left" ? this.config.yAxisLabelPadding : this.config.yAxisLabelPadding + k + q + this.config.quadrantPadding,
      y: T + a - (b ? _ / 2 : 0),
  ...
  getQuadrantPoints(n) {
    const { quadrantSpace: f } = n, { quadrantHeight: d, quadrantLeft: h, quadrantTop: p, quadrantWidth: y } = f, _ = ie().domain([0, 1]).range([h, y + h]), a = ie().domain([0, 1]).range([d + p, p]);
...
o(de, "setXAxisRightText");
function ue(t) {
  V.setData({ yAxisTopText: O(t.text) });
}
o(ue, "setYAxisTopText");
function xe(t) {
  V.setData({ yAxisBottomText: O(t.text) });
}
o(xe, "setYAxisBottomText");
function St(t) {
  const n = {};
  for (const f of t) {
...
function Te() {
  ...
  }), V.setData({ titleText: ae() }), V.build();
}
o(Te, "getQuadrantData");
var we = /* @__PURE__ */ o(function() {
  V.clear(), Le();
}, "clear"), Be = {
  setWidth: pe,
  setHeight: ye,
  ...
  setQuadrant4Text: he,
  setXAxisLeftText: ce,
  setXAxisRightText: de,
  setYAxisTopText: ue,
  setYAxisBottomText: xe,
  parseStyles: St,
  addPoint: fe,
  addClass: ge,
  getQuadrantData: Te,
...
` + t);
  ...
  let k;
  a === "sandbox" && (k = Lt("#i" + n));
  const T = (a === "sandbox" ? Lt(k.nodes()[0].contentDocument.body) : Lt("body")).select(`[id="${n}"]`), q = T.append("g").attr("class", "main"), m = _.quadrantChart?.chartWidth ?? 500, b = _.quadrantChart?.chartHeight ?? 500;
  Ce(T, b, m, _.quadrantChart?.useMaxWidth ?? !0), T.attr("viewBox", "0 0 " + m + " " + b), d.db.setHeight(b), d.db.setWidth(m);
  const g = d.db.getQuadrantData(), A = q.append("g").attr("class", "quadrants"), ht = q.append("g").attr("class", "border"), xt = q.append("g").attr("class", "data-points"), ft = q.append("g").attr("class", "labels"), gt = q.append("g").attr("class", "title");
  ...
  const ct = A.selectAll("g.quadrant").data(g.quadrants).enter().append("g").attr("class", "quadrant");
...
//# sourceMappingURL=quadrantDiagram-W4KKPZXB.js.map
