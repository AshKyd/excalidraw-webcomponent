import { _ as o, c as ot, a4 as ut, L as dt, al as yt, A as ft, k as pt, q as it, a as gt, b as kt, g as mt, s as wt, t as _t, e as bt } from "./index.js";
var J = (function() {
  var e = /* @__PURE__ */ o(function(T, t, s, i) {
    ...
    parse: /* @__PURE__ */ o(function(t) {
      ...
      function Y(B) {
        i.length = i.length - 2 * B, p.length = p.length - B, n.length = n.length - B;
      }
      o(Y, "popStack");
      function tt() {
        var B;
        return B = c.pop() || b.lex() || A, typeof B != "number" && (B instanceof Array && (c = B, B = c.pop()), B = s.symbols_[B] || B), B;
      }
      o(tt, "lex");
      for (var M, W, N, q, F = {}, U, V, et, Z; ; ) {
        if (W = i[i.length - 1], this.defaultActions[W] ? N = this.defaultActions[W] : ((M === null || typeof M > "u") && (M = tt()), N = v[W] && v[W][M]), typeof N > "u" || !N.length || !N[0]) {
          var K = "";
          Z = [];
          for (U in v[W])
...
Expecting ` + Z.join(", ") + ", got '" + (this.terminals_[M] || M) + "'" : K = "Parse error on line " + (I + 1) + ": Unexpected " + (M == A ? "end of input" : "'" + (this.terminals_[M] || M) + "'"), this.parseError(K, {
        ...
        switch (N[0]) {
          case 1:
            i.push(M), p.push(b.yytext), n.push(b.yylloc), i.push(N[1]), M = null, $ = b.yyleng, w = b.yytext, I = b.yylineno, R = b.yylloc;
            break;
          case 2:
            if (V = this.productions_[N[1]][1], F.$ = p[p.length - V], F._$ = {
              ...
              return q;
            V && (i = i.slice(0, -1 * V * 2), p = p.slice(0, -1 * V), n = n.slice(0, -1 * V)), i.push(this.productions_[N[1]][0]), p.push(F.$), n.push(F._$), et = v[i[i.length - 2]][i[i.length - 1]], i.push(et);
            break;
          case 3:
            return !0;
        }
...
var xt = J, H, vt = (H = class {
...
}, o(H, "IshikawaDB"), H), St = 14, j = 250, $t = 30, Et = 60, At = 5, ht = 82 * Math.PI / 180, st = Math.cos(ht), nt = Math.sin(ht), at = /* @__PURE__ */ o((e, h, r) => {
  ...
  let i = 0, c = j;
  const p = g ? void 0 : z(E, i, c, i, c, "ishikawa-spine");
  if (Lt(E, i, c, y.text, k, t), !_.length) {
    g && z(E, i, c, i, c, "ishikawa-spine", t), at(O, x, D);
    return;
  }
  i -= 20;
  const n = _.filter((S, P) => P % 2 === 0), v = _.filter((S, P) => P % 2 === 1), w = rt(n), I = rt(v), $ = w.total + I.total;
  let L = j, A = j;
  if ($ > 0) {
    const S = j * 2, P = j * 0.3;
  ...
  const C = k * 2;
  L = Math.max(L, w.max * C), A = Math.max(A, I.max * C), c = Math.max(L, j), p && p.attr("y1", c).attr("y2", c), E.select(".ishikawa-head-group").attr("transform", `translate(0,${c})`);
  ...
  for (let S = 0; S < b; S++) {
    ...
    for (const [R, G, Y] of [
      ...
      R && Pt(P, R, i, c, G, Y, k, t);
    i = P.selectAll("text").nodes().reduce((R, G) => Math.min(R, G.getBBox().x), 1 / 0);
  }
  if (g)
    z(E, i, c, 0, c, "ishikawa-spine", t);
  else {
    p.attr("x1", i);
    const S = `url(#${s})`;
  ...
  if (z(e, r, a, _, x, "ishikawa-branch", d), d && Q(e, r, a, r - _, a - x, d), Mt(e, h.text, _, x, l, f, d), !m.length)
    return;
  ...
  for (const [p, n] of D.entries()) {
    ...
    if (n.depth % 2 === 0) {
      const C = w.y1 - w.y0;
      $ = lt(w.x0, w.x1, C ? (v - w.y0) / C : 0.5), L = v, A = $ - (n.childCount > 0 ? Et + n.childCount * At : $t), z(I, $, v, A, v, "ishikawa-sub-branch", d), d && Q(I, $, v, 1, 0, d), X(I, n.text, A, v, "ishikawa-label align", "end", f);
    } else {
      const C = w.childrenDrawn++;
      $ = lt(w.x0, w.x1, (w.childCount - C) / (w.childCount + 1)), L = w.y0, A = $ + s * ((v - L) / i), z(I, $, L, A, v, "ishikawa-sub-branch", d), d && Q(I, $, L, $ - A, L - v, d), X(I, n.text, A, v, c, "end", f);
    }
...
`);
  ...
  const d = Bt(h), m = f * 1.05, u = e.append("text").attr("class", l).attr("text-anchor", y).attr("x", r).attr("y", a - (d.length - 1) * m / 2);
  for (const [k, g] of d.entries())
    u.append("tspan").attr("x", r).attr("dy", k === 0 ? 0 : m).text(g);
  return u;
}, "drawMultilineText"), lt = /* @__PURE__ */ o((e, h, r) => e + (h - e) * r, "lerp"), z = /* @__PURE__ */ o((e, h, r, a, l, y, f) => {
  if (f) {
...
//# sourceMappingURL=ishikawaDiagram-YF4QCWOH.js.map
