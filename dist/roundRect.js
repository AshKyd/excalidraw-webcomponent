(() => {
  ...
  function M(e, s, a, u, o) {
    ...
    if (x?.x < 0 ? x.x : x?.y, P.some(({ x: n, y: f }) => !Number.isFinite(n) || !Number.isFinite(f)))
      return;
    if (x)
      throw new RangeError(`${b(this)} Radius value ${x} is negative.`);
    D(P), a < 0 && u < 0 ? (this.moveTo(e - t.x, s), this.ellipse(e + a + r.x, s - r.y, r.x, r.y, 0, -Math.PI * 1.5, -Math.PI), this.ellipse(e + a + i.x, s + u + i.y, i.x, i.y, 0, -Math.PI, -Math.PI / 2), this.ellipse(e - h.x, s + u + h.y, h.x, h.y, 0, -Math.PI / 2, 0), this.ellipse(e - t.x, s - t.y, t.x, t.y, 0, 0, -Math.PI / 2)) : a < 0 ? (this.moveTo(e - t.x, s), this.ellipse(e + a + r.x, s + r.y, r.x, r.y, 0, -Math.PI / 2, -Math.PI, 1), this.ellipse(e + a + i.x, s + u - i.y, i.x, i.y, 0, -Math.PI, -Math.PI * 1.5, 1), this.ellipse(e - h.x, s + u - h.y, h.x, h.y, 0, Math.PI / 2, 0, 1), this.ellipse(e - t.x, s + t.y, t.x, t.y, 0, 0, -Math.PI / 2, 1)) : u < 0 ? (this.moveTo(e + t.x, s), this.ellipse(e + a - r.x, s - r.y, r.x, r.y, 0, Math.PI / 2, 0, 1), this.ellipse(e + a - i.x, s + u + i.y, i.x, i.y, 0, 0, -Math.PI / 2, 1), this.ellipse(e + h.x, s + u + h.y, h.x, h.y, 0, -Math.PI / 2, -Math.PI, 1), this.ellipse(e + t.x, s - t.y, t.x, t.y, 0, -Math.PI, -Math.PI * 1.5, 1)) : (this.moveTo(e + t.x, s), this.ellipse(e + a - r.x, s + r.y, r.x, r.y, 0, -Math.PI / 2, 0), this.ellipse(e + a - i.x, s + u - i.y, i.x, i.y, 0, 0, Math.PI / 2), this.ellipse(e + h.x, s + u - h.y, h.x, h.y, 0, Math.PI / 2, Math.PI), this.ellipse(e + t.x, s + t.y, t.x, t.y, 0, Math.PI, Math.PI * 1.5)), this.closePath(), this.moveTo(e, s);
    function R(n) {
