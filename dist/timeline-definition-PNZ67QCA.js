import { _ as o, F as gt, ac as Rt, ad as Ct, ae as Wt, c as pt, l as E, L as Pt, a4 as Bt, af as ft, d as X, E as Vt, ag as Ft, A as zt } from "./index.js";
import { d as ot } from "./arc.js";
var tt = (function() {
  var e = /* @__PURE__ */ o(function(k, s, d, l) {
    for (d = d || {}, l = k.length; l--; d[k[l]] = s) ;
    return d;
  }, "o"), t = [6, 11, 13, 14, 15, 17, 19, 20, 23, 24], n = [1, 12], i = [1, 13], r = [1, 14], h = [1, 15], c = [1, 16], a = [1, 19], f = [1, 20], p = {
    ...
    performAction: /* @__PURE__ */ o(function(s, d, l, g, x, u, S) {
      var v = u.length - 1;
      switch (x) {
        case 1:
          return u[v - 1];
    ...
    parse: /* @__PURE__ */ o(function(s) {
      var d = this, l = [0], g = [], x = [null], u = [], S = this.table, v = "", I = 0, R = 0, W = 2, O = 1, L = u.slice.call(arguments, 1), w = Object.create(this.lexer), H = { yy: {} };
      ...
      function b() {
        var A;
        return A = g.pop() || w.lex() || O, typeof A != "number" && (A instanceof Array && (g = A, A = g.pop()), A = d.symbols_[A] || A), A;
      }
      o(b, "lex");
    ...
    var k = {
      ...
      unput: /* @__PURE__ */ o(function(s) {
        ...
        return this.yylloc = {
          ...
          last_column: l ? (l.length === g.length ? this.yylloc.first_column : 0) + g[g.length - l.length].length - l[0].length : this.yylloc.first_column - d
...
` + d + "^";
      ...
      test_match: /* @__PURE__ */ o(function(s, d) {
        ...
        }, this.options.ranges && (x.yylloc.range = this.yylloc.range.slice(0))), g = s[0].match(/(?:\r\n?|\n).*/g), g && (this.yylineno += g.length), this.yylloc = {
      ...
      next: /* @__PURE__ */ o(function() {
        ...
        for (var x = this._currentRules(), u = 0; u < x.length; u++)
          if (l = this._input.match(this.rules[x[u]]), l && (!d || l[0].length > d[0].length)) {
            if (d = l, g = u, this.options.backtrack_lexer) {
        ...
        return d ? (s = this.test_match(d, x[g]), s !== !1 ? s : !1) : this._input === "" ? this.EOF : this.parseError("Lexical error on line " + (this.yylineno + 1) + `. Unrecognized text.
` + this.showPosition(), {
          text: "",
          token: null,
          line: this.yylineno
      ...
      performAction: /* @__PURE__ */ o(function(d, l, g, x) {
        switch (g) {
          case 0:
            break;
          case 1:
  ...
  return o(y, "Parser"), y.prototype = p, p.Parser = y, new y();
})();
tt.parser = tt;
var Ot = tt, yt = {};
Vt(yt, {
...
var D = "", mt = 0, nt = "LR", rt = [], q = [], K = [], xt = /* @__PURE__ */ o(() => Ft, "getCommonDb"), kt = /* @__PURE__ */ o(function() {
  ...
  function h(f) {
    const p = ot().startAngle(Math.PI / 2).endAngle(3 * (Math.PI / 2)).innerRadius(7.5).outerRadius(6.8181818181818175);
    f.append("path").attr("class", "mouth").attr("d", p).attr("transform", "translate(" + t.cx + "," + (t.cy + 2) + ")");
  }
  o(h, "smile");
  function c(f) {
    const p = ot().startAngle(3 * Math.PI / 2).endAngle(5 * (Math.PI / 2)).innerRadius(7.5).outerRadius(6.8181818181818175);
    f.append("path").attr("class", "mouth").attr("d", p).attr("transform", "translate(" + t.cx + "," + (t.cy + 7) + ")");
  }
  o(c, "sad");
  function a(f) {
  ...
  function e(r, h, c, a, f, p, m, y) {
    const k = h.append("text").attr("x", c + f / 2).attr("y", a + p / 2 + 5).style("font-color", y).style("text-anchor", "middle").text(r);
    i(k, m);
  }
  o(e, "byText");
  function t(r, h, c, a, f, p, m, y, k) {
    const { taskFontSize: s, taskFontFamily: d } = y, l = r.split(/<br\s*\/?>/gi);
    for (let g = 0; g < l.length; g++) {
      const x = g * s - s * (l.length - 1) / 2, u = h.append("text").attr("x", c + f / 2).attr("y", a).attr("fill", k).style("text-anchor", "middle").style("font-size", s).style("font-family", d);
      u.append("tspan").attr("x", c + f / 2).attr("dy", x).text(l[g]), u.attr("y", a + p / 2).attr("dominant-baseline", "central").attr("alignment-baseline", "central"), i(u, m);
    }
  }
  o(t, "byTspan");
  function n(r, h, c, a, f, p, m, y) {
    const k = h.append("switch"), d = k.append("foreignObject").attr("x", c).attr("y", a).attr("width", f).attr("height", p).attr("position", "fixed").append("xhtml:div").style("display", "table").style("height", "100%").style("width", "100%");
    d.append("div").attr("class", "label").style("display", "table-cell").style("text-align", "center").style("vertical-align", "middle").text(r), t(r, k, c, a, f, p, m, y), i(d, m);
  }
  o(n, "byFo");
  function i(r, h) {
    for (const c in h)
...
function it(e, t) {
  e.each(function() {
    var n = X(this), i = n.text().split(/(\s+|<br>)/).reverse(), r, h = [], c = 1.1, a = n.attr("y"), f = parseFloat(n.attr("dy")), p = n.text(null).append("tspan").attr("x", 0).attr("y", a).attr("dy", f + "em");
    for (let m = 0; m < i.length; m++)
      r = i[i.length - 1 - m], h.push(r), p.text(h.join(" ").trim()), (p.node().getComputedTextLength() > t || r === "<br>") && (h.pop(), p.text(h.join(" ").trim()), r === "<br>" ? h = [""] : h = [r], p = n.append("tspan").attr("x", 0).attr("y", a).attr("dy", c + "em").text(r));
  });
}
o(it, "wrap");
var Qt = /* @__PURE__ */ o(function(e, t, n, i, r, h = !1) {
  const { theme: c, look: a } = i, f = c?.includes("redux"), p = i?.themeVariables?.THEME_COLOR_LIMIT ?? 12, m = n % p - 1, y = e.append("g");
  t.section = m, y.attr(
    "class",
    (t.class ? t.class + " " : "") + "timeline-node " + ("section-" + m)
  );
  const k = y.append("g"), s = y.append("g"), l = s.append("text").text(t.descr).attr("dy", "1em").attr("alignment-baseline", "middle").attr("dominant-baseline", "middle").attr("text-anchor", "middle").call(it, t.width).node().getBBox(), g = i.fontSize?.replace ? i.fontSize.replace("px", "") : i.fontSize;
  if (t.height = l.height + g * 1.1 * 0.5 + t.padding, t.height = Math.max(t.height, t.maxHeight), t.width = t.width + 2 * t.padding, s.attr("transform", "translate(" + t.width / 2 + ", " + t.padding / 2 + ")"), f && s.attr(
    "transform",
  ...
  const r = pt(), { look: h, theme: c, themeVariables: a } = r, { useGradient: f, gradientStart: p, gradientStop: m } = a, y = r.timeline?.leftMargin ?? 50;
  ...
  l.append("g");
  const g = i.db.getTasks(), x = i.db.getCommonDb().getDiagramTitle();
  E.debug("task", g), M.initGraphics(l, t);
  ...
  E.debug("maxSectionHeight before draw", S), E.debug("maxTaskHeight before draw", v), u && u.length > 0 ? u.forEach((N) => {
    const b = g.filter((P) => P.section === N), _ = {
      number: L,
      descr: N,
      section: L,
  ...
  if (E.debug("bounds", F), x && l.append("text").text(x).attr("x", h === "neo" ? F.x * 2 + y : F.width / 2 - y).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), I = w ? S + v + 150 : v + 100, l.append("g").attr("class", "lineWrapper").append("line").attr("x1", y).attr("y1", I).attr("x2", F.width + 3 * y).attr("y2", I).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", `url(#${t}-arrowhead)`), h === "neo" && f && c !== "neutral") {
    ...
    _.append("stop").attr("offset", "0%").attr("stop-color", p).attr("stop-opacity", 1), _.append("stop").attr("offset", "100%").attr("stop-color", m).attr("stop-opacity", 1);
  }
  ft(
    void 0,
    l,
...
}, "draw"), lt = /* @__PURE__ */ o(function(e, t, n, i, r, h, c, a, f, p, m, y) {
  for (const k of t) {
    const s = {
      descr: k.task,
      section: n,
    ...
    E.debug("taskNode", s);
    const d = e.append("g").attr("class", "taskWrapper"), g = M.drawNode(d, s, n, c, y).height;
    if (E.debug("taskHeight after draw", g), d.attr("transform", `translate(${i}, ${r})`), h = Math.max(h, g), k.events) {
  ...
  const r = pt(), h = r.timeline?.leftMargin ?? 50;
  E.debug("timeline", i.db);
  const c = Pt(t);
  c.append("g");
  const a = i.db.getTasks(), f = i.db.getCommonDb().getDiagramTitle();
  E.debug("task", a), M.initGraphics(c);
  const p = i.db.getSections();
  E.debug("sections", p);
  let m = 0, y = 0;
  const k = 50 + h;
  let s = 50;
  const d = s, l = k, g = se + Mt, x = ie + At, u = l + g;
  let S = 0;
  const v = p && p.length > 0, I = v ? u : k + g, R = Math.max(50, g + x - z * 2);
  ...
  for (const p of t) {
    ...
    if (y.attr("transform", `translate(${d}, ${r})`), h = Math.max(h, s), p.events && p.events.length > 0) {
      const l = r, g = i + At;
      ce(e, p.events, n, i, g, l, c);
    }
  ...
  for (const f of t) {
    ...
    E.debug("eventNode", p);
    const m = e.append("g").attr("class", "eventWrapper"), k = M.drawNode(m, p, n, c).height;
  ...
  const { theme: t } = gt(), n = t?.includes("dark"), i = t?.includes("color"), r = e.svgId?.replace(/^#/, "") ?? "", h = r ? `url(#${r}-drop-shadow)` : e.dropShadow ?? "none";
  let c = "";
  for (let a = 0; a < e.THEME_COLOR_LIMIT; a++) {
    const f = `${17 - 3 * a}`, p = i ? e.borderColorArray[a] : e.mainBkg, m = i ? e.borderColorArray[a] : e.nodeBorder;
    c += `
    .section-${a - 1} rect,
    .section-${a - 1} path,
    .section-${a - 1} circle {
      fill: ${n && i ? e.mainBkg : p};
      stroke: ${m};
  ...
  const { theme: t } = gt(), n = t?.includes("redux"), i = t === "neutral", r = e.svgId?.replace(/^#/, "") ?? "";
  let h = "";
...
//# sourceMappingURL=timeline-definition-PNZ67QCA.js.map
